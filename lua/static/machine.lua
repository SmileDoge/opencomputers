...
local tooLongWithoutYielding = setmetatable({}, {
    __tostring = function()
...
do
    ...
    local string_find, string_lower, string_match, string_gmatch, string_gsub = string.find, string.lower, string.match,
    ...
    local function iscntrl(b)
        return b <= '\007' or (b >= '\010' and b <= '\017') or (b >= '\020' and b <= '\027') or
                   (b >= '\030' and b <= '\037' and b ~= ' ') or b == '\177'
    end
    ...
    local function ispunct(b)
        return
            (b >= '{' and b <= '~') or (b == '`') or (b >= '[' and b <= '_') or (b == '@') or (b >= ':' and b <= '?') or
                (b >= '(' and b <= '/') or (b >= '!' and b <= '\'')
    end
    ...
    local function match_capture(ms, s, l)
        ...
        if ms.src_end - s >= len and ms.capture[l].init:head(len) == s:head(len) then
            return s:copy(len)
        else
            return nil
        end
        ...
        if p ~= ms.p_end then
            ...
            elseif p0 == L_ESC then -- escaped sequences not in the format class[*+?-]?
                ...
                elseif p1 == 'f' then -- frontier?
                    ...
                    if not matchbracketclass(previous, p, ep:copy(-1)) and matchbracketclass(s:char(), p, ep:copy(-1)) then
                        p = ep
...
sandbox = {
    ...
    ipairs = ipairs,
    load = function(code, name, mode, env) return load(code, name, mode, env or sandbox) end,
...
local function processResult(result)
    ...
    --PrintTable(result)
    --print("faiufgmadoiugofisdgunsdfipogunsfpdgusdfp,io")
...
local directCache = setmetatable({}, {
    __mode = "k"
})
...
local function bootstrap()
    ...
    if eeprom then
        ...
        if code and #code > 0 then
            local bios = load(code, "=bios", "t", sandbox)
            if bios then
                return coroutine.create(bios), {
                    n = 0
                }
            end
            error("failed loading bios: " .. bios, 0)
        end
    end
